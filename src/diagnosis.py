...
def main_diagnosis(
    ...
    ## Handle dates
    date_bgn = datetime.datetime.strptime(
        str(ayyyy) + "-" + str(am).zfill(2) + "-" + str(ad).zfill(2), "%Y-%m-%d"
    )
    ...
    date_end = datetime.datetime.strptime(
        str(nayyyy) + "-" + str(nam).zfill(2) + "-01-00", "%Y-%m-%d-%H"
    )
    ...
    for ix in range(ntime):
        ...
        if verbose:
            print(
                " TOTAL: " + str(date_seq[ix]) + " has " + str(nparticle) + " parcels"
            )
        ...
        if fproc_npart:
            ...
            ary_npart = gridall(
                imidi[:, 1], imidi[:, 0], np.repeat(1, nparticle), glon=glon, glat=glat
            )
        ...
        if fprec:
            ...
            # grid
            ary_prec = gridall(
                pmidi[:, 1], pmidi[:, 0], dq[:, isprec][0], glon=glon, glat=glat
            )
            ary_pnpart = gridall(
                pmidi[:, 1],
                pmidi[:, 0],
                np.repeat(1, isprec.size),
                glon=glon,
                glat=glat,
            )
        ...
        ## Evaporation
        if fevap:
            isevap = filter_for_evap_parcels(
                ary,
                dq,
                cpbl_method,
                cpbl_strict,
            ...
            # grid
            ary_evap = gridall(
                emidi[:, 1], emidi[:, 0], dq[:, isevap][0], glon=glon, glat=glat
            )
            ary_enpart = gridall(
                emidi[:, 1],
                emidi[:, 0],
                np.repeat(1, isevap.size),
                glon=glon,
                glat=glat,
            )
        ...
        ## Sensible heat
        if fheat:
            isheat = filter_for_heat_parcels(
                ary,
                dTH,
                cpbl_method,
                cpbl_strict,
            ...
            # grid
            ary_heat = gridall(
                hmidi[:, 1], hmidi[:, 0], dTH[:, isheat][0], glon=glon, glat=glat
            )
            ary_hnpart = gridall(
                hmidi[:, 1],
                hmidi[:, 0],
                np.repeat(1, isheat.size),
                glon=glon,
                glat=glat,
            )
