...
def calc_stats(mdata, mndata, rdata, thresh=0.001):
    ...
    for y in range(rdata.shape[1]):  
        ...
        for x in range(rdata.shape[2]):  
            myctab   = calc_ctab_measures(contingency_table(rdata[:,y,x],mndata[:,y,x],thresh=thresh))
            acc[y,x] = myctab['acc']
            pod[y,x] = myctab['pod']
    ...
    return ({"acc":acc,"fbias":fbias,"pod":pod,"pofd":pofd,"pss":pss,"odr":odr,"diff":diff,"bias":bias})

def write_to_netcdf(ofile,vals,var="P"):
    print(" Writing "+str(var)+" validation statistics to output file: "+str(ofile))
...
def main_validation(
    ...
    ofilename = str(ofile_base)+"_diag_r"+str(ryyyy)[-2:]+"_"+str(ayyyy)+"-"+str(am).zfill(2)+"_validation.nc"
    ...
    if fevap:
        Eref, reflats, reflons = eraloader_12hourly(var='e',
                     ...
                     maskneg=False,
                     uptake_years=fyears,
                     uptake_dates=fdays, lats=totlats, lons=totlons)
